...
void pio_usb_device_prepare_control_data(uint8_t *data, uint8_t len) {
  prepare_ep0_data(data, len);
}

void pio_usb_device_prepare_control_rx(uint8_t *data, uint8_t len) {
  prepare_ep0_rx(data, len);
}

__attribute__((weak)) bool pio_usb_device_control_request_cb(uint8_t *setup_packet) {
  (void)setup_packet;
  return false;
}
...
static int __no_inline_not_in_flash_func(process_device_setup_stage)(uint8_t *buffer) {
  ...
  if (pio_usb_device_control_request_cb(buffer)) {
    return 0;
  }
